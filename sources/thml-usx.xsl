...
<xsl:template match="/">
    ...
    <usx version="3.0">
        <book code="{$bookID}" />
        <para style="h"><xsl:value-of select="$title"/></para>
        <para style="toc1"><xsl:value-of select="translate($mySectID,'+',' ')"/></para>
    ...
    <xsl:when test="$theDivID != '' and $start=''">
      <xsl:apply-templates select="/ThML/ThML.body/div1[@id=$theDivID] | /ThML/ThML.body/div1/div2[@id=$theDivID] | /ThML/ThML.body/div1/div2/div3[@id=$theDivID] | /ThML/ThML.body/div1/div2/div3/div4[@id=$theDivID] | /ThML/ThML.body/div1/div2/div3/div4/div5[@id=$theDivID]" mode="includeParsing"/>
    </xsl:when>

    <xsl:when test="$isDict=1 and ($myTerm!='' or $start!='')">
      <xsl:if test="$myTerm!=''">
        <xsl:choose>
        <xsl:when test="//glossary/term[translate(., $termTrans, 'abcdefghijklmnopqrstuvwxyz ')=$myTerm]">
